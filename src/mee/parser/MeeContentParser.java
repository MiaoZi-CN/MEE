...
public class MeeContentParser {
    ...
    LoadedMod currentMod = mods.getMod(MEE.class);
    ...
    public void markError(Content content, Fi file, Throwable error){
        ...
        if(currentMod != null){
            currentMod.erroredContent.add(content);
        }
    }
        ...
        for(JsonValue child = jsonMap.child; child != null; child = child.next){
            ...
            if(metadata == null){
                if(ignoreUnknownFields){
                    Log.warn("[@]:不可识别字段: @ (@)", currentContent.minfo.sourceFile.name(), child.name, type.getSimpleName());
    ...
    <T> Class<T> resolve(String base, Class<T> def){
        ...
        var out = MeeClassMap.classmap.get(!base.isEmpty() && Character.isLowerCase(base.charAt(0)) ? Strings.capitalize(base) : base);
